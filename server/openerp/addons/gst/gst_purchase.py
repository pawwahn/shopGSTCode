...
class gst_sales_search_record(osv.Model):
    _name = 'gst.sales.search.record'
    _columns = {
        'bill_date': fields.date('Bill Date', size=10, required=True),
        # 'firm': fields.selection(FIRM, 'Firm', select=True),
        'bill_num': fields.char('Bill Number', required=True, size=20),
        # 'financial_year': fields.many2one('gst.financial.years', 'Financial Year', required=True),
        'purchase_from_many_to_one': fields.many2one('gst.purchased.from.setup', required=True),
    }

    def is_record_found(self, cr, uid, ids, context=None):
        out = cStringIO.StringIO()
        datas = self.read(cr, uid, ids, [], context=context)[0]
        bill_date = datas['bill_date']
        bill_num = datas['bill_num']
        purchase_from = datas['purchase_from_many_to_one'][0]
        logging.info(purchase_from)
        logging.info(bill_date)
        logging.info(bill_num)
        logging.info(type(bill_num))
        logging.info("***********************")
        purchase_query = """ select * from gst_detail_purchase as gdp,gst_purchased_from_setup as gs where bill_num=%s 
                             and to_char(bill_date,'yyyy-mm-dd')=%s 
                             and purchase_from_many_to_one=%s and 
                             gdp.purchase_from_many_to_one = gs.id """

        cr.execute(purchase_query, (bill_num, bill_date, purchase_from))
        detail_results = cr.fetchall()
        #logging.info(purchase_query)
        logging.info("detail_results----- >> {}".format(detail_results))
        if detail_results:
            raise osv.except_osv(_('STATUS'), _('-- BILL FOUND  --'))
        else:
            raise osv.except_osv(_('STATUS'), _('-->> BILL IS NOT AVAILABLE  <<--'))
